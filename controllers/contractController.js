...
const getContractPartner = async (req, res) => {
  try {
    ...
    const user_id = userInfo.id;
    const getContractsInfo = await pool.query(
      "SELECT c.id,c.recommendation_id,c.invitation_sender_rating,c.recommendation_sender_rating,c.invitation_sender_cmt,c.recommendation_sender_cmt,i.start_time,i.end_time,i.date, r.food_recommend FROM contracts c INNER JOIN recommendations r on c.recommendation_id = r.id INNER JOIN invitations i ON r.invitation_id = i.id WHERE r.recommendation_sender_id = $1;",
      [user_id]
    );
  ...
  try {
    ...
    if (userInfo.role == "user") {
      const newReview = await pool.query(
        "INSERT INTO contracts (recommendation_sender_rating, recommendation_sender_cmt) VALUES ($1, $2) WHERE id = $3",
        [rate, comment, id]
      );
      res.json(newReview);
    }
    if (userInfo.role == "partner") {
      const newReview = await pool.query(
        "INSERT INTO contracts (invitation_sender_rating, invitation_sender_cmt) VALUES ($1, $2) WHERE id = $3",
        [rate, comment, id]
      );
      res.json(newReview);
    }
